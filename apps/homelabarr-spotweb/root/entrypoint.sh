...
if [ ! -f /config/ownsettings.php ] && [ -f /var/www/spotweb/ownsettings.php ]; then
  $(which cp) /var/www/spotweb/ownsettings.php /config/ownsettings.php
fi

$(which touch) /config/ownsettings.php && \
$(which chown) www-data:www-data /config/ownsettings.php && \
$(which rm) -rf /var/www/spotweb/ownsettings.php && \
$(which ln) -s /config/ownsettings.php /var/www/spotweb/ownsettings.php && \
$(which chown) -R www-data:www-data /var/www/spotweb &>/dev/null
...
if [[ -f "/config/dbsettings.inc.php" ]];then
   $(which chown) www-data:www-data /config/dbsettings.inc.php &>/dev/null
   $(which rm) -rf /var/www/spotweb/dbsettings.inc.php && \
   $(which ln) -s /config/dbsettings.inc.php /var/www/spotweb/dbsettings.inc.php
else
...
if [[ -n "$SPOTWEB_CRON_RETRIEVE" || -n "$SPOTWEB_CRON_CACHE_CHECK" ]]; then
   $(which ln) -sf /proc/$$/fd/1 /var/log/stdout
   service cron start
...
# Run database update
$(which php) /var/www/spotweb/bin/upgrade-db.php >/dev/null 2>&1

# Clean up apache pid (if there is one)
$(which rm) -rf /run/apache2/apache2.pid &>/dev/null
